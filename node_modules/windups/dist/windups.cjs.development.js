...
function useWindup(windupInit, options) {
  var _React$useReducer = React.useReducer(windupReducer, windupInit, initWindupState),
...
var WindupContext = /*#__PURE__*/React.createContext({
  skip: function skip() {
...
function useSkip() {
  var _React$useContext = React.useContext(WindupContext),
      skip = _React$useContext.skip;

  return skip;
}
function useRewind() {
  var _React$useContext2 = React.useContext(WindupContext),
      rewind = _React$useContext2.rewind;

  return rewind;
}
function useIsFinished() {
  var _React$useContext3 = React.useContext(WindupContext),
...
function reduceWindupArgs(prevArgs, children) {
  ...
  var newArgs = React.Children.toArray(childrenChildren).reduce(reduceWindupArgs, []);
...
function buildKeyString(children) {
  ...
  var keys = React.Children.map(children, function (child) {
...
function useChildrenMemo(factory, children) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  var memoChildren = React.useMemo(factory, [buildKeyString(children)]);
  return memoChildren;
}
...
function wrapChildren(children, Wrapper) {
  if (typeof children === "string") {
    return children.split("").map(function (_char, i) {
      return _char === "\n" ? _char : React.createElement(Wrapper, {
        key: _char + "-" + i
      }, _char);
    });
  }
  ...
  if (typeof children === "number") {
    return children.toString().split("").map(function (_char2) {
      return React.createElement(Wrapper, null, _char2);
    });
  }
  ...
  if (children.props.children) {
    ...
    return React.createElement(Outer, Object.assign({
      key: children.key
    }, children.props), React.Children.map(children.props.children, function (ch) {
...
var CharWrapper = function CharWrapper(_ref) {
  ...
  return React.createElement(React.Fragment, null, React.Children.map(children, function (ch) {
...
function makeGetDescriptorsOfChildren(defaultFontStyle) {
  return function (descriptors, children) {
    ...
    if (isStyledTextElement(children)) {
      return [].concat(descriptors, React.Children.toArray(children.props.children).reduce(makeGetDescriptorsOfChildren(children.props.fontStyle), []));
    }

    return [].concat(descriptors, React.Children.toArray(children.props.children).reduce(makeGetDescriptorsOfChildren(defaultFontStyle), []));
  };
}
...
function reinsertStringsIntoChildren(_ref, children) {
  ...
  var _React$Children$toArr = React.Children.toArray(children.props.children).reduce(reinsertStringsIntoChildren, [[], accStrings]),
      subChildrenAcc = _React$Children$toArr[0],
      subStringsAcc = _React$Children$toArr[1];

  return [[].concat(accChildren, [React.cloneElement(children, {
...
function textFromChildren(children) {
  var text = React.Children.map(children, function (ch) {
    if (typeof ch === "string") {
      return ch;
    }
...
exports.OnChar = OnChar;
exports.Pace = Pace;
exports.Pause = Pause;
exports.StyledText = StyledText;
exports.WindupChildren = WindupChildren;
exports.defaultGetPace = defaultGetPace;
exports.textFromChildren = textFromChildren;
exports.useIsFinished = useIsFinished;
